 ...
 * Ассемблер СВС-Б.
 *
 * Вызов: as [-uxXd] file.s [-o outfile]
 *
 * -u   - не считать неопределенное имя внешним
 * -x   - удалить из таблицы имен все локальные символы
 * -X   - удалить из таблицы имен локальные символы, начинающиеся с 'L'
 * -d   - флаг отладки
 * -b   - установить мнемонику Балакирева
 *
 * Автор: Вакуленко С. В.
 */
...
register char *argv[];
{
	...
	if (infile && ! freopen (infile, "r", stdin))
		uerror (MSG ("cannot open %s", "не могу открыть %s"), infile);
	if (! freopen (outfile, "w", stdout))
		uerror (MSG ("cannot open %s", "не могу открыть %s"), outfile);
...
startup ()
{
	...
	for (i=STEXT; i<SBSS; i++) {
		if (! (sfile [i] = fopen (tfilename, "w+")))
			uerror (MSG ("cannot open %s", "не могу открыть %s"),
				tfilename);
		unlink (tfilename);
...
pass1 ()
{
	...
	while ((clex = getlex (&cval)) != LEOF) {
		...
		case LNAME:
			...
			uerror (MSG ("bad command", "неверная команда"));
		case LACMD:
			switch (cval) {
			case TEXT:
				segm = STEXT;
			...
			break;
		default:
			uerror (MSG ("bad syntax", "плохой синтаксис"));
		}
...
makeascii ()
{
	...
	c = getlex (&cval);
	if (c != '"')
		uerror (MSG ("no .ascii parameter", "нет параметра .ascii"));
	n = 0;
	for (;;) {
		switch (c = getchar ()) {
		case EOF:
			uerror (MSG ("EOF in text string",
				"незакрытая текстовая строка"));
		case '"':
			break;
		case '\\':
			switch (c = getchar ()) {
...
getbitmask ()
{
	...
	if (v != SABS)
		uerror (MSG ("illegal expression in bit mask",
			"недопустимое выражение в битовой маске"));
	c = getlex (&v);
	if (c != ':' && c != '=')
		uerror (MSG ("illegal bit mask delimiter",
			"неверный разделитель в битовой маске"));
	compl = c == '=';
	b = getexpr (&v) - 1;
	if (v != SABS)
		uerror (MSG ("illegal expression in bit mask",
...
char *alloc (len)
{
	...
	if ((lastfree += len) > SPACESZ)
		uerror (MSG ("out of memory", "мало памяти"));
	return (space + r);
}
...
getterm ()
{
	...
	switch (getlex (&cval)) {
	default:
		uerror (MSG ("operand missed", "нет операнда"));
	case LNUM:
		return (SABS);
	case LNAME:
...
typerel (t)     /* преобразование типа символа в тип настройки */
{
	switch (t & N_TYPE) {
