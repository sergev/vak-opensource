...
char type [] = {
	...
	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,
	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,	ALPH,
	ALPH,   ALPH,   ALPH,   ALPH,   ALPH,   ALPH,   ALPH,   0,
};

unsigned char token [TOKS];
char    eoa [] = "\0";
...
# endif
	if (! strcmp (argv [1], "-o")) {
		argc -= 2;
		argv += 2;
		if (! freopen (*argv, "w", stdout)) {
			fprintf (stderr, "m4: cannot write to: %s\n", *argv);
			delexit();
		}
	}
	if (argc>1)
