...
static int evaluate_unknown(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_pld(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	/* PLD */
...
static int evaluate_srs(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	...
	case 0x08400000:
		snprintf(instruction->text, 128, "0x%8.8x"
				"\t0x%8.8x"
				"\tSRS%s\tSP%s, #%d",
				address, opcode,
				mode, wback,
...
static int evaluate_swi(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	instruction->type = ARM_SWI;

	snprintf(instruction->text, 128,
			"0x%8.8x\t0x%8.8x\tSVC %#6.6x",
			address, opcode, (opcode & 0xffffff));

	return 0;
}

static int evaluate_blx_imm(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	int offset;
	unsigned immediate;
...
static int evaluate_b_bl(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	unsigned char L;
	unsigned immediate;
...
static int evaluate_ldc_stc_mcrr_mrrc(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
/* both normal and extended instruction space (condition field b1111) */
static int evaluate_cdp_mcr_mrc(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	const char *cond;
	char* mnemonic;
...
static int evaluate_load_store(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_media(unsigned opcode, unsigned address,
...
static int evaluate_misc_load_store(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_ldm_stm(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_mul_and_extra_ld_st(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_mrs_msr(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_misc_instr(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
	/* MRS/MSR */
...
static int evaluate_data_proc(unsigned opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
int arm_evaluate_opcode(unsigned opcode, unsigned address,
		struct arm_instruction *instruction)
{
	...
	uerror("ARM: should never reach this point (opcode=%08x)",
			(unsigned) opcode);
	return -1;
}

static int evaluate_b_bl_blx_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_add_sub_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_shift_imm_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
	unsigned char Rd = (opcode >> 0) & 0x7;
	...
	snprintf(instruction->text, 128,
			"0x%8.8x  0x%4.4x    \t%s\tr%i, r%i, #%#2.2x" ,
			address, opcode, mnemonic, Rd, Rm, imm);

	return 0;
}

static int evaluate_data_proc_imm_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
	...
	snprintf(instruction->text, 128,
			"0x%8.8x  0x%4.4x    \t%s\tr%i, #%#2.2x",
			address, opcode, mnemonic, Rd, imm);

	return 0;
}

static int evaluate_data_proc_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_load_literal_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
	unsigned immediate;
	unsigned char Rd = (opcode >> 8) & 0x7;
...
static int evaluate_load_store_reg_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
	unsigned char Rd = (opcode >> 0) & 0x7;
	unsigned char Rn = (opcode >> 3) & 0x7;
...
static int evaluate_load_store_imm_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_load_store_stack_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_add_sp_pc_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_adjust_stack_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_breakpoint_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
	unsigned imm = opcode  & 0xff;
	...
	snprintf(instruction->text, 128,
			"0x%8.8x  0x%4.4x  \tBKPT\t%#2.2x",
			address, opcode, imm);

	return 0;
}

static int evaluate_load_store_multiple_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
...
static int evaluate_cond_branch_thumb(unsigned short opcode,
		unsigned address, struct arm_instruction *instruction)
{
	unsigned offset = opcode  & 0xff;
	...
	else if (cond == 0xe)
	{
		instruction->type = ARM_UNDEFINED_INSTRUCTION;
		snprintf(instruction->text, 128,
			"0x%8.8x  0x%4.4x    \tUNDEFINED INSTRUCTION",
			address, opcode);
		return 0;
	}
		...
		struct arm_instruction *instruction)
{
	...
	snprintf(instruction->text, 128,
			"0x%8.8x  0x%4.4x    \tCB%sZ\tr%d, %#8.8x",
			address, opcode,
			(opcode & 0x0800) ? "N" : "",
		...
		struct arm_instruction *instruction)
{
	...
	if ((opcode & 0x0ff0) == 0x0650)
		snprintf(instruction->text, 128,
				"0x%8.8x  0x%4.4x    \tSETEND %s",
				address, opcode,
				(opcode & 0x80) ? "BE" : "LE");
	else /* ASSUME (opcode & 0x0fe0) == 0x0660 */
		snprintf(instruction->text, 128,
				"0x%8.8x  0x%4.4x    \tCPSI%c\t%s%s%s",
				address, opcode,
				(opcode & 0x0010) ? 'D' : 'E',
				(opcode & 0x0004) ? "A" : "",
				(opcode & 0x0002) ? "I" : "",
...
int thumb_evaluate_opcode(unsigned short opcode, unsigned address, struct arm_instruction *instruction)
{
	/* clear fields, to avoid confusion */
	memset(instruction, 0, sizeof(struct arm_instruction));
	instruction->opcode = opcode;
...
static int t2ev_b_bl(unsigned opcode, unsigned address,
...
static int t2ev_cond_b(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	unsigned offset;
	unsigned b17 = 1 << 17;
...
static int t2ev_hint(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	const char *mnemonic;
...
static int t2ev_misc(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	const char *mnemonic;
...
static int t2ev_b_misc(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
...
static int t2ev_data_immed(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	char *mnemonic = NULL;
	int rn = (opcode >> 16) & 0xf;
...
static int t2ev_store_single(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
...
static int t2ev_mul32(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
...
static int t2ev_mul64_div(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	int op = (opcode >> 4) & 0xf;
	char *infix = "MUL";
...
static int t2ev_ldm_stm(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	int rn = (opcode >> 16) & 0xf;
...
static int t2ev_ldrex_strex(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
...
literal:
	...
	if (opcode & (1 << 23))
		...
		address -= imm;
	sprintf(cp, "%s\tr%u, r%u, %#8.8x",
			mnemonic, rt, rd, address);
	return 0;
}

static int t2ev_data_shift(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
...
static int t2ev_data_reg(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	char *mnemonic;
	char * suffix = "";
...
static int t2ev_load_word(unsigned opcode, unsigned address,
		struct arm_instruction *instruction, char *cp)
{
	int rn = (opcode >> 16) & 0xf;
	int immed;
	...
	unsigned short (*target_read_u16) (unsigned))
{
	...
	uerror("Can't decode 32-bit Thumb2 yet (opcode=%08x)",
			opcode);
