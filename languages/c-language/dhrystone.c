 ...
 * iMac i5 4-core  2.7GHz       OS X 10.8.1     gcc -O3       49309k  49309k
 *
 * NOTE
...
-- Stop Timer --
*****************/
        gettimeofday (&benchtime, 0);

        msec = (benchtime.tv_sec - starttime.tv_sec) * 1000;
        msec += (benchtime.tv_usec - starttime.tv_usec) / 1000;
	printf("Dhrystone(%s) time for %ld passes = %.2f sec\n",
		Version, (long) LOOPS, msec / 1000.0);
	printf("This machine benchmarks at %ld dhrystones/msec\n",
		((long) LOOPS) / msec);
}
