...
int send_ufi_command(int fd, unsigned char *cdb, int cdb_len, unsigned char *data, int data_len,
                     int direction)
{
    sg_io_hdr_t io_hdr;
...
void print_format_capacities(unsigned char *data, int len)
{
    ...
    while (offset + 8 <= len && list_len >= 8) {
        unsigned int blocks = (data[offset] << 24) | (data[offset + 1] << 16) |
        ...
        switch (type) {
        case 0:
            type_str = "Unformatted";
            break;
        case 1:
            type_str = "Formatted";
            break;
        case 2:
            type_str = "Formattable";
            break;
        default:
            type_str = "Reserved";
            break;
...
void print_read_capacity(unsigned char *data)
{
    ...
    printf("Media Capacity: %u blocks of %u bytes (%u KB total)\n", blocks + 1, block_size,
           ((blocks + 1) * block_size) / 1024);
}

void print_mode_sense_data(unsigned char *data, int len)
{
    if (len < 8)
        return; // MODE SENSE(10) header is 8 bytes
...
int main(int argc, char *argv[])
{
    ...
    // MODE SENSE (10-byte)
    unsigned char mode_sense_cdb[10] = {
        0x5A, 0, 0x3F, 0, 0, 0, 0, (255 >> 8) & 0xFF, 255 & 0xFF, 0
    }; // PC=0, All pages
    memset(data, 0, sizeof(data));
