...
void print_read_capacity(unsigned char *data) {
    ...
    printf("Media Capacity: %u blocks of %u bytes (%llu KB total)\n", blocks + 1, block_size, ((blocks + 1LL) * block_size) / 1024);
}
...
void print_mode_sense_data(unsigned char *data, int len) {
    ...
    int offset = 4;  // Skip header (4 bytes for 6-byte MODE SENSE, but UFI uses 10-byte)
    while (offset < len) {
