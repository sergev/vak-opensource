...
static int sg_io(int fd, uint8_t *cdb, int cdb_len, void *dxferp, unsigned int dxfer_len,
                 int direction, uint8_t *sense, unsigned int timeout_ms)
{
...
} geometry_table[] = { { 1440, 80, 2, 18, 512 }, { 1232, 77, 2, 8, 1024 }, { 1200, 80, 2, 15, 512 },
                       { 720, 80, 2, 9, 512 },   { 640, 80, 2, 8, 512 },   { 0, 0, 0, 0, 0 } };

static int size_to_geometry(unsigned int kb, uint16_t *cylinders, uint8_t *heads,
                            uint8_t *sec_per_track, uint16_t *bytes_per_sec)
{
...
static int do_format_unit(int fd, uint32_t num_blocks, uint32_t block_len, uint8_t track,
                          uint8_t head)
{
    uint8_t cdb[UFI_CDB_LEN] = { 0 };
...
static void sense_message(uint8_t *sense, size_t len, char *buf, size_t bufsiz)
{
    ...
    if (asc == 0x3a)
        ...
        asc_str = " (invalid format)";
    else if (asc == 0x27 && ascq == 0)
        asc_str = " (write protected)";
...
static int infer_geometry(uint8_t medium_type, uint32_t num_blocks, uint32_t block_len,
                          uint16_t *cylinders, uint8_t *heads, uint8_t *sec_per_track,
                          uint16_t *bytes_per_sec)
{
    ...
    if (num_blocks == 1440 && block_len == 512) {
        *cylinders     = 80;
        *heads         = 2;
        *sec_per_track = 9;
        *bytes_per_sec = 512;
    ...
    if (num_blocks == 2880 && block_len == 512) {
        *cylinders     = 80;
        *heads         = 2;
        *sec_per_track = 18;
        *bytes_per_sec = 512;
    ...
    if (num_blocks == 1232 && block_len == 1024) {
        *cylinders     = 77;
        *heads         = 2;
        *sec_per_track = 8;
        *bytes_per_sec = 1024;
    ...
    if (num_blocks == 2400 && block_len == 512) {
        *cylinders     = 80;
        *heads         = 2;
        *sec_per_track = 15;
        *bytes_per_sec = 512;
...
static int run_format(int fd, unsigned int capacity_kb, int force, int verify, uint8_t *fmt_buf,
                      size_t fmt_len)
{
    ...
    for (uint16_t t = 0; t < cylinders; t++) {
        for (uint8_t h = 0; h < heads; h++) {
            ...
            if (r != 0) {
                ...
                if (rs != 0) {
                    fprintf(stderr,
                            "%s: FORMAT UNIT failed at track=%u head=%u; REQUEST SENSE also failed "
                            "(rc=%d)\n",
                            progname, (unsigned)t, (unsigned)h, rs);
                } else {
                    char msg[128];
                    sense_message(sense, SENSE_LEN, msg, sizeof(msg));
                    fprintf(stderr, "%s: FORMAT UNIT failed at track=%u head=%u: %s\n", progname,
                            (unsigned)t, (unsigned)h, msg);
                }
                return 1;
            }
        }
...
static void print_info(int fd, uint8_t *inq, size_t inq_len, uint8_t *fmt_buf, size_t fmt_len)
{
    if (inq_len >= 36) {
        printf("Vendor:   %.8s\n", inq + 8);
        printf("Product:  %.16s\n", inq + 16);
            ...
            if (avail >= 8) {
                ...
                while (off + 2 <= avail) {
                    ...
                    if (page_code == 0x05 && page_len >= 10) {
                        /* Flexible Disk Page (UFI Table 19): heads@4, sec/trk@5, bytes/sec@6-7,
                         * cyl@8-9 */
                        uint8_t heads         = mode_buf[off + 4];
                        uint8_t sec_per_track = mode_buf[off + 5];
                        uint16_t bytes_per_sec =
                            ((uint16_t)mode_buf[off + 6] << 8) | mode_buf[off + 7];
                        uint16_t cylinders = ((uint16_t)mode_buf[off + 8] << 8) | mode_buf[off + 9];
                        printf(
                            "Geometry: %u cylinders, %u heads, %u sectors/track, %u bytes/sector\n",
                            (unsigned)cylinders, (unsigned)heads, (unsigned)sec_per_track,
                ...
                if (!printed_geom) {
                    uint16_t cylinders = 0, bytes_per_sec = 0;
                    uint8_t heads = 0, sec_per_track = 0;
                    if (infer_geometry(medium_type, cur_blocks, cur_block_len, &cylinders, &heads,
                                       &sec_per_track, &bytes_per_sec)) {
                        printf(
                            "Geometry: %u cylinders, %u heads, %u sectors/track, %u bytes/sector\n",
                            (unsigned)cylinders, (unsigned)heads, (unsigned)sec_per_track,
                            (unsigned)bytes_per_sec);
    ...
    switch (desc_code) {
    case 0x01:
        desc_type = "Unformatted (max)";
        break;
    case 0x02:
        desc_type = "Formatted (current)";
        break;
    case 0x03:
        desc_type = "No media (max)";
        break;
    }
    printf("\nCapacity: %s - %u blocks x %u bytes\n", desc_type, (unsigned)num_blocks,
           (unsigned)block_len);
    int num_fmt = list_len >= 8 ? (list_len - 8) / 8 : 0;
    ...
    for (int i = 0; i < num_fmt; i++) {
        ...
        if (nb == 1440 && bl == 512)
            name = "720 KB";
        else if (nb == 2880 && bl == 512)
            name = "1.44 MB";
        else if (nb == 1232 && bl == 1024)
            name = "1.25 MB";
        else if (nb == 2400 && bl == 512)
            name = "1.2 MB";
        else if (nb == 640 && bl == 512)
            name = "640 KB";
...
int main(int argc, char **argv)
{
    ...
    static struct option longopts[] = { { "help", no_argument, NULL, 'h' }, { NULL, 0, NULL, 0 } };
    while ((opt = getopt_long(argc, argv, "if:FVh", longopts, NULL)) != -1) {
        ...
        case 'i':
            ...
            break;
        case 'f':
            capacity_kb = (unsigned int)atoi(optarg);
            break;
        case 'F':
            force = 1;
            break;
