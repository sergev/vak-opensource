...
int nexttoken(void)
{
    ...
    if (strchr("0123456789.", *input)) {
        ...
        if (isfunc("ABS") || isfunc("ACOS") || isfunc("ASIN") || isfunc("ATAN") || isfunc("COSH") ||
            isfunc("COS") || isfunc("EXP") || isfunc("LOG10") || isfunc("LOG") || isfunc("POW10") ||
            isfunc("ROUND") || isfunc("SINH") || isfunc("SIN") || isfunc("SQRT") || isfunc("SQR") ||
...
//
// Error callback function pointer - set by spreadsheet module
static void (*parser_error_callback)(const char *) = NULL;

void set_parser_error_callback(void (*callback)(const char *))
{
    parser_error_callback = callback;
}
...
void push(struct TOKENREC *token)
{
    if (stacktop == PARSERSTACKSIZE - 1) {
        if (parser_error_callback)
            parser_error_callback(MSGSTACKERROR);
        error = true;
    } else
...
// Uses get_spreadsheet() to access the current spreadsheet instance
//
