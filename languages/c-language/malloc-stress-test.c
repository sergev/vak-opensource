    ...
    if (debug_dump) {
        for (mx = 0; mx < malloc_calls_made; mx++) {
            printf("Buffer index %d: Address = 0x%llx, Size=%d, Fill=%d ", mx,
                   (unsigned long long)(minfo_array[mx].buf_ptr), minfo_array[mx].buf_size,
                   minfo_array[mx].set_val);
...
int check_overlap(void *p1, int len1, void *p2, int len2)
{
     ...
     */
    if ((bob1 >= bob2 && bob1 <= eob2) || (eob1 >= bob2 && eob1 <= eob2) ||
        (bob2 >= bob1 && bob2 <= eob1) || (eob2 >= bob1 && eob2 <= eob1)) {
        printf(
            "FAIL:  Buffers overlap:  Buffer 1 (0x%llx, %d bytes), Buffer 2 (0x%llx, %d bytes)\n",
            bob1, len1, bob2, len2);
...
void do_test(void)
{
    ...
    for (;;) {
        ...
        for (bx = 0; bx < malloc_calls_made; bx++) {
             ...
             */
            if (check_overlap(malloc_buf, buf_size, minfo_array[bx].buf_ptr,
                              minfo_array[bx].buf_size)) {
                summarize();
