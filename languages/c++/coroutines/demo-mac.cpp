...
// Use clang++ 12.0 to compile on Mac.
//
#include <iostream>
...
public:
    ...
    co_void_t(promise_type &promise)
        : handle(std::experimental::coroutine_handle<promise_type>::from_promise(promise))
    {
    }

    // Return the handle, cast to void value.
    operator std::experimental::coroutine_handle<>() const { return handle; }
...
int main()
{
    std::experimental::coroutine_handle<> continuation = counter();
    for (int i = 0; i < 5; ++i) {
