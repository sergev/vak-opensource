...
int mydbm_compare (register const DBT *a, register const DBT *b)
{
	if (a->size != b->size)
...
DBM *mydbm_open (const char *file, int flags, int mode)
{
	BTREEINFO info;
	...
	info.minkeypage = 3;
	info.psize = 4096;
	info.compare = mydbm_compare;
	info.prefix = 0;
	info.lorder = 0;
	return (dbopen (file, flags, mode, DB_BTREE, &info));
}
#endif
...
int flags, mode;
{
	...
	if (access (db->basefile, 0) != 0) {
		if (flags != (O_RDWR | O_CREAT))
			goto err1;
		/* Нет такого файла, создаем новый DBM-файл */
		db->dbm = dbm_open (db->basefile, O_RDWR | O_CREAT, db->mode);
		if (! db->dbm) {
err1:                   close (db->fd);
			free ((char *) db->tab);
			free (db->basefile);
	...
	/*
	 * Открываем DBM на запись с целью заблокировать
	 * дальнейшие обращения.  На самом деле
	 * писать туда не будем.
	 */
	cdbm_error = 7;
	db->dbm = dbm_open (db->basefile, O_RDWR, db->mode);
	if (! db->dbm) {
		close (db->fd);
		free ((char *) db->tab);
		free (db->basefile);
...
register CDBM *db;
{
	...
	newdbm = dbm_open (newname, O_RDWR | O_CREAT, db->mode);
	if (! newdbm)
		goto ret;
