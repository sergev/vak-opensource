#!/usr/bin/env python
#
# Convert data from narcc.org to a configuration for baoclone.
# Select active voice channels only.
#
import sys, getopt
...
#
def usage():
    print "Usage: narcc-to-baoclone [-a activity.txt] filename.txt..."
...
#
# Check whether the repeater is active: voice, callsign or test.
#
def is_active(mhz):
    global activity
    try:
        active = get_activity(mhz)
        if active.find("voice") >= 0:
            return 1
        if active.find("callsign") >= 0:
            return 1
        if active.find("test") >= 0:
            return 1
        return 0
    except:
        return 0
