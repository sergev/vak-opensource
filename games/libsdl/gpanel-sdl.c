...
int gpanel_open(const char *devname)
{
    ...
    _gpanel_screen = SDL_SetVideoMode(WIDTH, HEIGHT, 16, SDL_SWSURFACE);

    /* If we fail, return error. */
    if (! _gpanel_screen) {
        fprintf(stderr, "Unable to set %ux%u mode: %s\n",
...
int gpanel_get_pixel(int x, int y)
{
    ...
    if (SDL_MUSTLOCK(_gpanel_screen))
        if (SDL_LockSurface(_gpanel_screen) < 0)
            return 0;

    /* Draw to screen */
    ptr = (short*) (_gpanel_screen->pixels + y * _gpanel_screen->pitch);
    color = ptr[x];

    /* Unlock if needed */
    if (SDL_MUSTLOCK(_gpanel_screen))
        SDL_UnlockSurface(_gpanel_screen);
...
void gpanel_pixel(int color, int x, int y)
{
    ...
    SDL_UpdateRect(_gpanel_screen, x, y, 1, 1);
}
...
void gpanel_fill(int color, int x0, int y0, int x1, int y1)
{
    ...
    /* Tell SDL to update the whole screen */
    SDL_UpdateRect(_gpanel_screen, x0, y0, x1-x0+1, y1-y0+1);
}
    ...
    const unsigned short *bits)
{
    ...
    if (background >= 0) {
        ...
        for (h=0; h<font->height; h++) {
            ptr = (short*) (_gpanel_screen->pixels + (y + h) * _gpanel_screen->pitch);
    ...
    /* Tell SDL to update the whole screen */
    SDL_UpdateRect(_gpanel_screen, x, y, width, font->height);
}
